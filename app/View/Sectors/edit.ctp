...
<div class="col-sm-2">
	<div class="control-side text-center">
			...
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $this->Form->value('Sector.id')), null, __('Are you sure you want to delete # %s?', $this->Form->value('Sector.id'))); ?>
		</button></div>
			<div><button class="btn btn-primary" >
		<?php echo $this->Html->link(__('List Sectors'), array('action' => 'index')); ?>
		</button></div>
		</div></div>
...
<div ba-panel="" ba-panel-title="Standard Fields" ba-panel-class="with-scroll">
    ...
    <div class="panel-heading clearfix"><h3 class="panel-title"><?php echo __('Edit Sector'); ?></h3>
    </div>
    <div class="panel">
