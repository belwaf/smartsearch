 ...
 */
	public function testRequestWithQueryStringArguments() {
		$url = '/profiles/search/sort:User.name/direction:desc?nome=&cpm=&audience=public';
		$result = $this->Jquery->request($url);
		$expected = '$.ajax({url:"\\/profiles\\/search\\/sort:User.name\\/direction:desc?nome=&cpm=&audience=public"});';
		$this->assertEquals($expected, $result);
	}
